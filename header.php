...
$app = new DemoLib();
    ...
    $stmt = $app->runQuery("SELECT * FROM users WHERE user_id=:user_id");
    $stmt->execute(array(":user_id"=>$user_id));
    $userRow=$stmt->fetch(PDO::FETCH_ASSOC);

    $stmt1 = $app->runQuery("SELECT * FROM review ORDER BY id DESC");
    $stmt1->execute();

    $stmt3 = $app->runQuery("SELECT * FROM categories");
    $stmt3->execute();

    $stmt31 = $app->runQuery("SELECT * FROM categories");
    $stmt31->execute();
